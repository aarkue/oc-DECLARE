...
pub fn oc_pn_prefilter(
    ...
    for (ot, from, from_mult, to, to_mult) in places {
        ...
        net.add_arc(
            ...
            Some(if from_mult { 10 } else { 1 }),
        );
        net.add_arc(
            ArcType::place_to_transition(place_id, *to),
            Some(if to_mult { 10 } else { 1 }),
        );
    }
    ...
    serde_json::to_writer_pretty(File::create(format!("./oc-pn-{}.json",name)).unwrap(), &oc_pn).unwrap();
    arcs
}
...
mod test {
    ...
    fn test_reduction() {
        ...
        let new_ret = oc_pn_prefilter("nina",res, &locel, noise_thresh, &act_ob_inv);
